...
--
-- NOTE: Supabase CLI executes migrations using prepared statements. Some SQL
-- parsers struggle to split files containing multiple function definitions +
-- grants/comments, which can lead to:
--   "cannot insert multiple commands into a prepared statement"
-- To avoid that, this migration is a single SQL statement (DO block) that
-- executes each DDL statement via EXECUTE.
...
BEGIN
  ...
  -- =====================================================
  EXECUTE $sql$
    CREATE OR REPLACE FUNCTION public.consume_video_credit_atomically(
      p_user_id uuid,
      p_youtube_id text,
      p_identifier text,
  ...
  EXECUTE $sql$
    COMMENT ON FUNCTION public.consume_video_credit_atomically(
      uuid,
      text,
      text,
      text,
  ...
  EXECUTE $sql$
    GRANT EXECUTE ON FUNCTION public.consume_video_credit_atomically(
      uuid,
      text,
      text,
      text,
  ...
  -- =====================================================
  EXECUTE $sql$
    CREATE OR REPLACE FUNCTION public.check_video_generation_allowed(
      p_user_id uuid,
      p_base_limit integer,
      p_period_start timestamptz,
      p_period_end timestamptz
  ...
  EXECUTE $sql$
    COMMENT ON FUNCTION public.check_video_generation_allowed(
      uuid,
      integer,
      timestamptz,
      timestamptz
  ...
  EXECUTE $sql$
    GRANT EXECUTE ON FUNCTION public.check_video_generation_allowed(
      uuid,
      integer,
      timestamptz,
      timestamptz
