...
--
-- NOTE: This migration is a single SQL statement (DO block) so it can be safely
-- executed by runners that use prepared statements.
...
BEGIN
  ...
  EXECUTE $sql$
    CREATE INDEX IF NOT EXISTS idx_video_generations_user_youtube_period
      ON public.video_generations (user_id, youtube_id, created_at)
  ...
  EXECUTE $sql$
    COMMENT ON FUNCTION public.consume_video_credit_atomically(
      uuid, text, text, text, integer, timestamptz, timestamptz, uuid, boolean
    ) IS
